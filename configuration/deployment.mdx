...
description: "Step-by-step guide to deploying SCO Autolavados using Docker Compose, including database setup, migrations, seeding, and verifying service health."
---
...
<Steps>
  ...
  <Step title="Run database migrations">
    ...
    This command runs inside the `sco_api` container and executes all pending migrations in `backend/prisma/migrations` against the live PostgreSQL instance.
  </Step>

  <Step title="Seed initial data">
    Populate the database with seed data (roles, default configuration, etc.):

    ```bash
    docker compose exec backend npx prisma db seed
    ```

    The seed script is defined in `backend/package.json` under the `prisma.seed` key. On first boot, `initializeApp()` in `src/index.ts` also creates the `AutoLavado` record automatically if none exists.
  </Step>
  ...
  <Step title="Verify services are healthy">
    ...
    A successful response returns the `AutoLavado` entity JSON, confirming that the database connection, migrations, and seed data are all working correctly. The frontend is accessible at `http://localhost:5173`.
  </Step>
</Steps>
