...
<ParamField path="CHOKIDAR_USEPOLLING" type="boolean" default="false">
  Enables filesystem polling in development environments where inotify-based watching is unreliable (such as Docker on Windows or macOS with virtualized file systems).
  This variable is already hardcoded to `true` in `docker-compose.yml` for the `backend` service — you do not need to set it manually.
</ParamField>
